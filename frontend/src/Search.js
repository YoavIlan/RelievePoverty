...
class Search extends Component {
  constructor(props) {
    ...
    this.state = {
      states: { data: [], total: 0 },
      news: { data: [], total: 0 },
      charities: { data: [], total: 0 },
      api:
        "https://api.relievepoverty.me/v1/search/" +
  ...
  async componentWillMount() {
    await this.getJSON(
      this.state.api +
        "?news_page=" +
        this.state.news_page +
        "&states_page=" +
        this.state.states_page +
        "&charities_page=" +
        this.state.charities_page
    ).then(response => {
  ...
  async componentWillReceiveProps(next) {
    ...
    await this.getJSON(
      "https://api.relievepoverty.me/v1/search/" +
        next.match.params.query +
        "?news_page=1&states_page=1&charities_page=1"
    ).then(response => {
      this.setState(JSON.parse(JSON.stringify(response)));
    });
  }
  ...
  render() {
    ...
    return (
      <>
        <Jumbotron
          title={"Search for Charities that Help to Relieve Poverty in the US"}
          description={
            "Charities throughout the US are doing great work every single day to combat poverty. Help them accomplish their goals by donating today."
          }
