...
describe("<States />", () => {
  ...
  it("filters by median income correctly", async () => {
    ...
    inst.setState({
      filters: {
        median_income_low: "median_income_low=40000",
        median_income_high: "median_income_high=50000"
      }
    });
    await inst.accessAPI();
  ...
  it("filters by poverty rate correctly", async () => {
    ...
    inst.setState({
      filters: {
        below_poverty_rate_low: "below_poverty_rate_low=05",
        below_poverty_rate_high: "below_poverty_rate_high=10"
      }
    });
    await inst.accessAPI();
  ...
  it("filters by child poverty rate correctly", async () => {
    ...
    inst.setState({
      filters: {
        child_poverty_rate_low: "child_poverty_rate_low=05",
        child_poverty_rate_high: "child_poverty_rate_high=15"
      }
    });
    await inst.accessAPI();
...
describe("<Charities />", () => {
  ...
  it("filters by tax code correctly", async () => {
    ...
    await inst.componentWillMount();
    inst.setState({
      filters: { tax_classification: "tax_classification=501(c)(8)" }
    });
    await inst.accessAPI();
    expect(wrapper.state().total).toEqual(1);
  });
...
describe("<NewsCard />", () => {
  ...
  it("renders NewCard image", () => {
    ...
    expect(actualImage.html()).toBe(
      '<img class="card-img-top d-flex" src="./img/colin.jpg" alt="Card cap"/>'
    );
  });
});
...
describe("<StatesCard />", () => {
  ...
  it("renders StatesCard rank at 19", () => {
    ...
    expect(actualRank.html()).toBe(
      "<p>19th out of 50 states for its poverty rate</p>"
    );
  });
  ...
  it("renders StatesCard rank at 1", () => {
    ...
    const actualRank = wrapper.find("p").first();
    expect(actualRank.html()).toBe(
      "<p>1st out of 50 states for its poverty rate</p>"
    );
  });
});
...
describe("<CharitiesCard />", () => {
  ...
  it("renders CharitiesCard state", () => {
    const wrapper = shallow(
      <CharitiesCard id="5" median_income="10" name="Charity" state="Texas" />
    );
    const actualState = wrapper.find("li").first();
    expect(actualState.html()).toBe(
      '<li><b>State:</b> <span><span class="">Texas</span></span></li>'
    );
  });
  ...
  it("renders CharitiesCard image", () => {
    ...
    expect(actualState.html()).toBe(
      '<img class="card-img-top d-flex" src="my_image" alt="Card"/>'
    );
  });
});
